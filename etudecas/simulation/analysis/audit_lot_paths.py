...
DEPLETION_EVENTS = {"production_consume", "lane_ship", "demand_service", "supplier_writeoff"}
...
def transport_kind(src: str, dst: str, node_type: dict[str, str]) -> str:
    ...
    if src_type == "supplier_dc" and dst_type == "factory":
        return "supplier_to_semifinished_site" if dst == "SDC-1450" else "supplier_to_factory"
    if src_type == "factory" and dst_type == "factory":
        return "semifinished_to_factory" if src == "SDC-1450" or dst == "SDC-1450" else "factory_to_factory"
...
def main() -> None:
    ...
    for receipt_key, parent_qty in transport_receipt_parent_qty.items():
        ...
        elif qty_mismatch(parent_qty, receipt_qty):
            ...
            issues.append(
                {
                    ...
                    "node_id": node_id,
                    "item_id": item_id,
                    "details": f"source_id={source_id} sum_parent_qty={parent_qty:.6f} receipt_qty={receipt_qty:.6f}",
                }
            )
...
- Transport receipt quantity mismatches: `{transport_receipt_qty_mismatches}`
...
- Supplier-material lots reaching a customer service event: `{len(supplier_mp_to_customer)}`
- Factory opening-stock lots reaching a customer service event: `{len(factory_opening_to_customer)}`
- Produced lots reaching a customer service event: `{len(production_to_customer)}`
