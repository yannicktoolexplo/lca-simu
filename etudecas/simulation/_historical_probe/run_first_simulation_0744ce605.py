...
def profile_value(profile: Any, day: int) -> float:
    if isinstance(profile, list):
        points = profile
        repeat_period = 0
    else:
        points = (profile or {}).get('points') or []
        repeat_period = int(round(max(0.0, to_float((profile or {}).get('repeat_period_days'), 0.0))))
    if not points:
        return 0.0
    if repeat_period > 0:
        day = day % repeat_period
    current = float(to_float(points[0].get('value'), 0.0))
    for point in points:
        point_day = int(round(to_float(point.get('t'), 0.0)))
        if day < point_day:
            break
        current = float(to_float(point.get('value'), 0.0))
    return current
