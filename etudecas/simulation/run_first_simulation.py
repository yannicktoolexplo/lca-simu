...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Simulation horizon in days. If 0, uses scenario horizon or 30.",
    )
    parser.add_argument(
        "--map-script",
...
def try_generate_plots(
    ...
    for r in input_stock_rows:
        ...
        item = str(r["item_id"])
        by_factory_item_day[factory][item][day] = to_float(r["stock_before_production"], 0.0)
    ...
    for factory, item_map in sorted(by_factory_item_day.items()):
        ...
        plt.title(f"Stocks d'entree de production par matiere premiere - {factory}")
        plt.xlabel("Jour")
        plt.ylabel("Stock avant production (unites)")
        plt.grid(alpha=0.3)
...
def main() -> None:
    ...
    for day in range(sim_days):
        ...
        for n in nodes:
            ...
            for p in (n.get("processes") or []):
                ...
                out_d0 = propagated_demand_daily.get(out_pair, 0.0)
                if out_d0 > 0:
                    out_target = max(base_stock.get(out_pair, 0.0), 7.0 * out_d0)
                    needed_out = max(0.0, out_target - stock[out_pair])
                    qty = min(qty, needed_out)
                if qty <= 0:
                    continue
        ...
        for pair, lane_list in lanes_by_dest_item.items():
            ...
            if d0 > 0:
                target = max(target, 7.0 * d0 + backlog[pair])
            ...
            for lane in lane_list:
                ...
                pipeline[arrival_day].append((dst, item_id, qty, lane["edge_id"]))
                in_transit[pair] += qty
                remaining -= qty
                shipped_today += qty
                transport_cost_today += qty * lane["unit_transport_cost"]
    ...
    summary = {
        ...
        "kpis": {
            ...
            "total_cost": round(total_transport_cost + total_holding_cost, 4),
        },
    ...
    with input_stock_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["day", "node_id", "item_id", "stock_before_production"])
        writer.writeheader()
    ...
    for r in input_stock_rows:
        ...
        per_day_values[day][key] = to_float(r["stock_before_production"], 0.0)
...
"""
    ...
    if generated_plots:
        ...
        print("[INFO] Plot generation skipped (matplotlib unavailable).")
