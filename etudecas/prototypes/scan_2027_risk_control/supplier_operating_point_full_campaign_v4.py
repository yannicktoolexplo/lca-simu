#!/usr/bin/env python3
"""Run the additive V4 three-state supplier incident campaign, shard by shard.
...
independent design seed 900659036 by maximizing the minimum common exposure
among ratio-admissible candidates, then frozen across all three states and the
30 fresh V4 holdout seeds.  Each paired repetition contains one baseline and two supplier
...
* ``planned_delivery_shortfall`` multiplies planned delivery reliability by
  0.5 (half the normally deliverable quantity, i.e. planned quantity times
  nominal delivery reliability, conditional on a dispatch).
...
This is therefore a conditional high-exposure stress design, not a random or
typical supplier incident sample.

The full design is 3 operating points x 18 lanes x 2 incidents x 30 seeds,
plus 3 x 30 paired baselines (3,330 reported rows), preceded by only three new
J720 design runs.  The 90 already validated V4 holdout shipment traces are
imported; they are never rerun for discovery.  Case horizons are adaptive.  The operating point remains
...
SCHEMA_VERSION = v4_contract.CAMPAIGN_SCHEMA_VERSION
CASE_SCHEMA_VERSION = v4_contract.CASE_SCHEMA_VERSION
PROGRESS_SCHEMA_VERSION = f"{SCHEMA_VERSION}.progress.v1"
PREFLIGHT_SCHEMA_VERSION = f"{SCHEMA_VERSION}.state_validation_binding.v1"
...
HOLDOUT_ACCEPTED_STATUS = "accepted_v4_holdout_bound_no_rerun"
HOLDOUT_REJECTED_STATUS = "rejected_v4_holdout_binding"
...
CONTRACT_REVISION = "v4_fresh30_imported_trace_fixed42d_adaptive_probe_v1_2026_09_05"
PREFLIGHT_BOOTSTRAP_REPLICATES = 10_000
PREFLIGHT_BOOTSTRAP_SEED = 20260905
...
DEFAULT_OUTPUT = ARTIFACT_ROOT / "supplier_operating_point_full_campaign_v4_20260905_v1"

OPERATING_POINT_IDS = ("op_100", "op_93", "op_80")
# The campaign reuses the accepted fresh V4 holdout seeds for paired incident
# inference.  The incident-window design seed was reserved before that holdout.
SEEDS = v4_contract.CAMPAIGN_SEEDS
TARGET_DESIGN_SEED = v4_contract.INCIDENT_DESIGN_SEED
SEED_BLOCK_SIZE = v4_contract.SEED_BLOCK_SIZE
SEED_BLOCKS = v4_contract.SEED_BLOCKS
...
MECHANISMS = (
    ...
    Mechanism(
        ...
        value=0.5,
        unit="multiplicateur_de_fiabilite_nominale_de_livraison",
        label_fr=(
            "Stress fournisseur de 6 semaines : 50 % de la quantité normalement "
            "livrable (plan × fiabilité nominale)"
        ),
    ),
)
...
def _validate_pending_multiseed_source(
    ...
    """Validate only the additive accepted V4 bridge and all 90 official traces."""

    if (
        payload.get("schema_version") != v4_contract.BRIDGE_SCHEMA_VERSION
        or payload.get("status") != v4_contract.BRIDGE_ACCEPTED_STATUS
    ):
        raise ValueError(
            "The V4 campaign requires the accepted validated_operating_points_v4 "
            "bridge; pending V1--V3 selections are intentionally unsupported"
        )
    validated = v4_bridge.validate_bridge(path.resolve(), revalidate_source=True)
    if validated != payload:
        raise ValueError("V4 bridge validator returned a different payload")
    cohorts = dict(payload.get("cohorts") or {})
    if (
        tuple(
            cohorts.get("campaign_repetitions_reuse_v4_fresh_holdout") or ()
        )
        != SEEDS
        or cohorts.get("incident_window_design_reserved") != [TARGET_DESIGN_SEED]
        or cohorts.get(
        ...
        raise ValueError("Accepted V4 bridge seed/no-retuning contract changed")
    source = dict(payload.get("source") or {})
    return {
        "producer": "v4_fresh_holdout_bridge",
        "plan_path": str(Path(str(source["plan_dir"])).resolve()),
        "plan_manifest_path": str(Path(str(source["plan_manifest"])).resolve()),
        "plan_signature": str(source["plan_signature"]),
        "selection_path": str(
            Path(str(source["development_selection"])).resolve()
        ),
        "selection_signature": str(source["development_selection_signature"]),
        "holdout_path": str(Path(str(source["holdout_result"])).resolve()),
        "holdout_signature": str(source["holdout_signature"]),
        "holdout_contract": dict(payload["holdout_contract"]),
        "trace_index_signature": str(payload["trace_index_signature"]),
        "trace_count": len(payload["trace_index"]),
    }
...
def _mechanism_contract() -> list[dict[str, Any]]:
    ...
    if {row["risk_type"] for row in result} & FORBIDDEN_INCIDENT_RISK_TYPES:
        raise AssertionError("The V4 mechanism contract contains a forbidden risk type")
    if {row["key"] for row in result} != {
        "transport_delay",
        "planned_delivery_shortfall",
    }:
        raise AssertionError("The V4 mechanism contract changed")
    return result
...
def _design_payload(
    *,
    ...
    if source_hashes.get("engine_sha256") != _sha256_file(engine) or source_hashes.get(
        ...
        raise ValueError(
            "Campaign engine/profile differ from the accepted signed V4 holdout"
        )
    ...
    holdout_path = Path(source_chain["holdout_path"])
    if (
        ...
        raise FileNotFoundError(
            "Signed V4 plan/selection/holdout evidence disappeared after validation"
        )
    ...
    bridge_lane_contract = dict(operating_point_source.get("lane_contract") or {})
    if (
        bridge_lane_contract.get("count") != len(lanes)
        or bridge_lane_contract.get("lanes")
        != v4_contract.lane_contract_payload(lane_projection)
        or bridge_lane_contract.get("sha256")
        != v4_contract.lane_contract_sha256(lane_projection)
    ):
        raise ValueError(
            "Campaign lane reference differs from the 18-lane V4 trace contract"
        )
    shard_projection = []
    for point_id in OPERATING_POINT_IDS:
    ...
    return {
        ...
        "operating_points_selection_signature": str(
            source_chain["selection_signature"]
        ),
        "operating_points_holdout": str(holdout_path.resolve()),
        "operating_points_holdout_sha256": _sha256_file(holdout_path),
        "operating_points_holdout_signature": str(source_chain["holdout_signature"]),
        "operating_points_holdout_contract": dict(source_chain["holdout_contract"]),
        "operating_points_trace_index_signature": str(
            source_chain["trace_index_signature"]
        ),
        "operating_points_trace_count": int(source_chain["trace_count"]),
        ...
        "expected_counts": {
            "auxiliary_discovery_runs": len(points),
            "design_window_engine_runs": len(points),
            "operating_point_validation_engine_runs": 0,
            "imported_v4_holdout_service_proofs": len(points) * len(SEEDS),
            "imported_v4_holdout_shipment_traces": len(points) * len(SEEDS),
        ...
        "operating_point_preflight_contract": {
            ...
            "timing": "before_the_three_design_runs_and_before_any_incident_probe",
            "campaign_seed_count": len(SEEDS),
            "design_seed_excluded": TARGET_DESIGN_SEED,
            "aggregation": "reuse_exact_recomputed_v4_fresh_holdout_result",
            ...
            "new_engine_runs": 0,
            "imported_service_proof_count": len(points) * len(SEEDS),
            "imported_shipment_trace_count": len(points) * len(SEEDS),
            "retuning": False,
            "failure_policy": "block_design_runs_and_all_incident_probes",
        },
        "target_selection": {
            ...
            "selection_rule": (
                "fixed_on_independent_design_seed_900659036_not_selected_on_campaign_repetitions; "
                "among_ratio_admissible_windows_maximise_the_minimum_cross_state_exposure; "
                "this_is_a_high_exposure_stress_window_not_a_typical_or_random_incident; "
            ...
            "cross_operating_point_identity": (
                "same_lane_specific_42_day_dates_are_fixed_on_design_seed_900659036_and_"
                "reused_across_all_states_and_30_campaign_seeds; ratio_le_1.5_is_admissible_"
                "but_quantities_and_physical_doses_are_not_equal"
            ),
        },
        "impact_metric_contract": {
            ...
            "normalization": (
                ...
                "component physical dose is reported separately and supports effect_per_"
                "1000_dose_alongside_total_impact"
            ),
        ...
        "cross_state_interpretation": (
            "conditional_on_one_balanced_product_supplier_planned_lead_degradation_"
            "family_and_lane_specific_high_exposure_windows_not_generic_supplier_states"
        ),
        "dynamic_scope_limit": (
            "stocks_transits_mrp_production_and_backlog_are_dynamic_but_explicit_dynamic_"
            "mrp_demand_coupling_covers_only_three_site_item_pairs_not_all_18_lanes"
        ),
        "stochastic_scope": {
            "random_sources": [
                {
...
def build_cross_state_target_registry(
    *,
    ...
    unsigned = {
        "schema_version": f"{SCHEMA_VERSION}.target_registry.v1",
        ...
        "disruption_window_days": INCIDENT_DISRUPTION_DAYS,
        "selection_contract": (
            "lane_specific_fixed_42d_window_selected_only_on_design_seed_900659036; "
    ...
    return {
        ...
        **dict(target),
        "interpretation_fr": (
            ...
            "conception 900659036 pour maximiser l'exposition minimale commune sous le seuil "
            "de ratio 1,5, puis appliquée aux trois états et aux 30 répétitions. C'est une "
            "fenêtre de forte exposition, pas un incident moyen ou aléatoire; le seuil rend "
            "la comparaison admissible sans rendre les doses identiques. "
...
def build_risk_row(
    *,
    ...
    if mechanism.risk_type in FORBIDDEN_INCIDENT_RISK_TYPES:
        raise ValueError(f"Forbidden V4 risk type: {mechanism.risk_type}")
    day = int(target["target_window_start_day"])
    end_day = int(target["target_window_end_day"])
    event_id = (
        f"v4__{point_id}__seed_{seed}__{lane.lane_id}__{mechanism.key}"
...
def _case_signature(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "v4_holdout_signature": manifest.get(
            "operating_points_holdout_signature", ""
        ),
        "v4_trace_index_signature": manifest.get(
            "operating_points_trace_index_signature", ""
        ),
        "state_validation_binding_signature": manifest.get(
            "state_validation_binding_signature", ""
        ),
        "execution_scope": manifest.get("execution_scope", "campaign_shard"),
...
def validate_incident_trace(
    *,
    ...
    if mechanism.key == "planned_delivery_shortfall":
        ...
        errors.append(f"unsupported V4 mechanism: {mechanism.key}")
...
def _build_v4_state_validation_binding(
    *, manifest: Mapping[str, Any], bridge: Mapping[str, Any]
) -> dict[str, Any]:
    """Bind the accepted V4 holdout without manufacturing a second holdout."""

    source = dict(bridge.get("source") or {})
    holdout = dict(bridge.get("holdout_contract") or {})
    states = holdout.get("state_summaries")
    if (
        bridge.get("status") != v4_contract.BRIDGE_ACCEPTED_STATUS
        ...
        or set(states) != set(OPERATING_POINT_IDS)
    ):
        raise ValueError("Accepted V4 state validation cannot be bound to campaign")
    unsigned = {
        "schema_version": PREFLIGHT_SCHEMA_VERSION,
        "contract_revision": CONTRACT_REVISION,
        "campaign_signature": manifest["campaign_signature"],
        "status": HOLDOUT_ACCEPTED_STATUS,
        "operating_points_input_status": manifest["operating_points_input_status"],
        "operating_points_artifact_signature": manifest[
            "operating_points_artifact_signature"
        ],
        "v4_plan_signature": source["plan_signature"],
        "v4_development_selection_signature": source[
            "development_selection_signature"
        ],
        "v4_holdout_signature": source["holdout_signature"],
        "v4_trace_index_signature": bridge["trace_index_signature"],
        "campaign_seeds": list(SEEDS),
        "campaign_seed_count": len(SEEDS),
        "design_seed": TARGET_DESIGN_SEED,
        "design_seed_in_acceptance_statistics": False,
        ...
        "source_holdout_used_once_without_retuning": True,
        "retuning_after_holdout": False,
        "states": states,
        ...
        "interpretation": (
            "Exact binding to the accepted fresh V4 holdout; no observed supplier "
            "performance and no additional operating-point test."
        ),
    }
    return {**unsigned, "binding_signature": _stable_sha256(unsigned)}


def _import_v4_holdout_shipment_rows(
    *,
    bridge_path: Path,
    ...
    """Read the 90 signed compact traces without rerunning a holdout case."""

    run_dir = Path(str(dict(bridge["source"])["run_dir"])).resolve()
    point_by_id = {str(point["operating_point_id"]): dict(point) for point in points}
    lane_by_id = {lane.lane_id: lane for lane in lanes}
    lane_ids = list(lane_by_id)
    lane_contract_rows = [asdict(lane) for lane in lanes]
    lane_sha = v4_contract.lane_contract_sha256(lane_contract_rows)
    filter_contract = v4_contract.trace_filter_contract(lane_contract_rows)
    imported: dict[tuple[str, int], list[dict[str, Any]]] = {}
    for index_row in bridge.get("trace_index") or []:
        point_id = str(index_row.get("operating_point_id") or "")
        seed = _as_int(index_row.get("seed"), -1)
        if point_id not in point_by_id or seed not in SEEDS:
            raise ValueError("V4 bridge trace index contains an unknown state/seed")
        expected = {
            "plan_signature": dict(bridge["source"])["plan_signature"],
            "candidate_key": index_row["candidate_key"],
            "candidate_id": index_row["candidate_id"],
            "target_group": point_id,
            "seed": seed,
            "graph_sha256": point_by_id[point_id]["graph_sha256"],
            "engine_sha256": dict(bridge["source_hashes"])["engine_sha256"],
            "simulation_days": DISCOVERY_DAYS,
            "lane_contract_sha256": lane_sha,
            "filter_contract": filter_contract,
        }
        _reference, trace = v4_contract.validate_trace_reference(
            index_row["shipment_trace"],
            run_dir=run_dir,
            expected=expected,
            allowed_lane_ids=lane_ids,
        )
        converted: list[dict[str, Any]] = []
        for raw in trace["rows"]:
            values = dict(zip(v4_contract.TRACE_ROW_FIELDS, raw, strict=True))
            lane = lane_by_id[str(values["lane_id"])]
            converted.append(
                {
                    "day": values["release_day"],
                    "shipment_id": values["shipment_id"],
                    "risk_decision_day": values["risk_decision_day"],
                    "risk_event_ids": "",
                    ...
                    "pulled_qty": values["pulled_qty"],
                    "lead_days": values["lead_days"],
                    "arrival_day": values["arrival_day"],
        ...
        key = (point_id, seed)
        if key in imported:
            raise ValueError("Duplicate state/seed V4 compact shipment trace")
        imported[key] = converted
    expected_keys = {
        (point_id, seed) for point_id in OPERATING_POINT_IDS for seed in SEEDS
    }
    if set(imported) != expected_keys:
        raise ValueError("V4 compact shipment trace matrix is not exactly 90 cases")
    return imported
...
def run_target_discovery(
    *,
    ...
    """Run three design cases and combine them with 90 retained V4 traces."""
    ...
    if (
        ...
        raise ValueError("V4 bridge differs from the signed campaign manifest")
    binding = _build_v4_state_validation_binding(manifest=manifest, bridge=bridge)
    ...
    if binding_path.is_file():
        if _read_json(binding_path) != binding:
            raise ValueError("Existing V4 state-validation binding changed")
    else:
        _write_json_atomic(binding_path, binding)

    progress_path = discovery_dir / "progress.json"
    jobs = [(point, TARGET_DESIGN_SEED) for point in points]
    ...
    def write_progress(status: str) -> None:
        _write_json_atomic(
            progress_path,
            {
                ...
                "campaign_signature": manifest["campaign_signature"],
                "status": status,
                "engine_runs_planned": 3,
                "engine_runs_completed": len(completed),
                "engine_runs_failed": len(failures),
                "running": max(0, min(workers, len(jobs) - len(completed) - len(failures))),
                "design_baselines_planned": 3,
                "design_baselines_completed": len(completed),
                "v4_service_proofs_imported": 90,
                "v4_shipment_traces_imported": 90,
                "v4_holdout_engine_reruns": 0,
    ...
    if failures:
        ...
        raise RuntimeError("V4 design discovery failed closed: " + json.dumps(failures))

    shipment_rows = _import_v4_holdout_shipment_rows(
        bridge_path=bridge_path,
        bridge=bridge,
        points=points,
        lanes=lanes,
    )
    shipment_rows.update(
        {key: list(value["shipment_rows"]) for key, value in completed.items()}
    )
    ...
    if registry_path.is_file():
        if _read_json(registry_path) != registry:
            raise ValueError("Existing target registry differs from V4 discovery")
    else:
    ...
    binding_manifest_fields = {
        ...
        "state_validation_binding_sha256": _sha256_file(binding_path),
    ...
    if registry.get("campaign_exposure_gate_passed") is not True:
        ...
        raise RuntimeError(
            ...
            f"exposure for every lane; no incident probe was started. See {registry_path}"
        )
    current_manifest.update(
        {
            **binding_manifest_fields,
...
def load_target_registry(
    *, output_dir: Path, manifest: Mapping[str, Any], lanes: Sequence[Lane]
    ...
    if (
        ...
        or manifest.get("state_validation_binding_status") != HOLDOUT_ACCEPTED_STATUS
    ):
        raise ValueError(
            "Target discovery requires the signed accepted V4 holdout binding"
        )
    binding_path = Path(
        str(manifest.get("state_validation_binding") or "")
    ).resolve()
    if not binding_path.is_file() or _sha256_file(binding_path) != str(
        manifest.get("state_validation_binding_sha256") or ""
    ):
        raise ValueError(
            "Signed V4 state-validation binding is missing or changed"
        )
    binding = _read_json(binding_path)
    unsigned_binding = dict(binding)
    binding_signature = str(unsigned_binding.pop("binding_signature", ""))
    if (
        binding.get("schema_version") != PREFLIGHT_SCHEMA_VERSION
        or binding.get("contract_revision") != CONTRACT_REVISION
        or binding.get("status") != HOLDOUT_ACCEPTED_STATUS
        ...
        raise ValueError("V4 state-validation binding fails its signed contract")
    ...
    if (
        registry.get("schema_version") != f"{SCHEMA_VERSION}.target_registry.v1"
        ...
        or registry.get("campaign_seeds") != list(SEEDS)
        ...
        != str(manifest.get("target_registry_sha256") or "")
        or signature != str(manifest.get("target_registry_signature") or "")
    ):
    ...
    if (
        ...
        or len(target_by_key) != expected
        or set(target_by_key) != expected_keys
        or set(contract_by_lane) != set(lane_by_id)
    ):
        raise ValueError("Cross-state target registry target matrix is incomplete")
    for lane_id, lane in lane_by_id.items():
        lane_contract = contract_by_lane[lane_id]
        fixed_start = _as_int(lane_contract.get("fixed_window_start_day"), -1)
        fixed_end = _as_int(lane_contract.get("fixed_window_end_day"), -1)
        design_quantities = lane_contract.get("design_quantities")
        if (
            fixed_start < 0
            or fixed_end - fixed_start + 1 != INCIDENT_DISRUPTION_DAYS
            or fixed_end >= STATE_EVALUATION_DAYS
            or not isinstance(design_quantities, Mapping)
            or set(design_quantities) != set(OPERATING_POINT_IDS)
        ):
            raise ValueError("Cross-state lane design window is malformed")
        design_values = [
            _as_float(design_quantities[point_id], math.nan)
            for point_id in OPERATING_POINT_IDS
        ]
        design_ratio = (
            max(design_values) / min(design_values)
            if min(design_values) > 1e-12
            else math.inf
        )
        if (
            any(not math.isfinite(value) or value <= 1e-12 for value in design_values)
            or design_ratio
            > STATE_MATCH_MAX_QUANTITY_RATIO + 1e-12
            or not math.isclose(
                _as_float(lane_contract.get("design_quantity_ratio"), math.nan),
                design_ratio,
                rel_tol=1e-10,
                abs_tol=1e-12,
            )
        ):
            raise ValueError("Cross-state lane design quantities are inconsistent")
        comparable_count = 0
        for seed in SEEDS:
            seed_rows = [
                target_by_key[(point_id, seed, lane_id)]
                for point_id in OPERATING_POINT_IDS
            ]
            quantities = [
                _as_float(row.get("target_planned_qty"), math.nan)
                for row in seed_rows
            ]
            if any(not math.isfinite(value) or value < 0.0 for value in quantities):
                raise ValueError("Cross-state registered quantity is invalid")
            positive = min(quantities) > 1e-12
            ...
            for row in seed_rows:
                if (
                    _as_int(row.get("target_window_start_day"), -1)
                    != fixed_start
                    or _as_int(row.get("target_window_end_day"), -1) != fixed_end
                    or _as_int(row.get("target_window_days"), -1)
                    != INCIDENT_DISRUPTION_DAYS
                    or _truthy(row.get("seed_cross_state_exposure_comparable"))
                    != comparable
                    or _as_int(row.get("required_comparable_seed_count"), -1)
                    != 24
                    or str(row.get("cross_state_match_status") or "")
                    != lane_contract.get("design_status")
                ):
                    raise ValueError(
                        "Cross-state target cell differs from its signed lane contract"
                    )
                raw_ratio = row.get("cross_state_quantity_ratio", "")
                if positive:
                    if not math.isclose(
                        _as_float(raw_ratio, math.nan),
                        ratio,
                        rel_tol=1e-10,
                        abs_tol=1e-12,
                    ):
                        raise ValueError(
                            "Cross-state target quantity ratio is inconsistent"
                        )
                elif str(raw_ratio).strip():
                    raise ValueError(
                        "Zero-exposure target cannot declare a finite quantity ratio"
                    )
        if (
            comparable_count
            != _as_int(lane_contract.get("comparable_campaign_seed_count"), -1)
            or comparable_count < 24
            or lane_contract.get("state_comparison_valid") is not True
        ):
            raise ValueError("Cross-state lane holdout exposure gate changed")
        for point_id in OPERATING_POINT_IDS:
            for seed in SEEDS:
                row = target_by_key[(point_id, seed, lane_id)]
                if (
                    _as_int(row.get("comparable_campaign_seed_count"), -1)
                    != comparable_count
                    or _truthy(row.get("state_comparison_valid"))
                    != (comparable_count >= 24)
                ):
                    raise ValueError(
                        "Cross-state target cell summary differs from recomputation"
                    )
    return registry
...
def _flatten_metric_row(
    ...
    *,
    ...
    affected_shipped = _as_float(proof.get("incident_affected_shipped_qty"), 0.0)
    if is_incident and mechanism_key == "planned_delivery_shortfall":
        ...
        effective_dose_qty = ""
        # The delayed physical dose is the quantity that actually ships, not
        # the upstream pull before nominal delivery reliability is applied.
        effective_dose_qty_days = 120.0 * affected_shipped
    else:
...
def run_shard(
    *,
    ...
    local_manifest["execution_scope"] = (
        "smoke_non_reusable" if smoke_seed is not None else "campaign_shard"
    )
    ...
    shard_contract = {
        ...
        "execution_scope": local_manifest["execution_scope"],
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Accepted signed V4 bridge that revalidates the official 30-seed holdout "
            "and its 90 compact shipment traces. No default is used so an obsolete "
            "operating-point artifact cannot be planned accidentally."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional frozen-seed override; default is the first comparable op_93 case.",
    )
...
def select_smoke_case(
    ...
    *,
    ...
    """Choose the first frozen op_93 case with positive comparable exposure."""

    if requested_seed is not None and requested_seed not in SEEDS:
        raise ValueError("Smoke seed must belong to the frozen V4 campaign cohort")
    lane_order = {lane_id: index for index, lane_id in enumerate(registry["lanes"])}
    ...
    candidates = [
        ...
        and (requested_seed is None or int(row.get("seed") or -1) == requested_seed)
        and (
            requested_lane_id is None
            or str(row.get("lane_id") or "") == requested_lane_id
        )
        and row.get("seed_cross_state_exposure_comparable") is True
        and str(row.get("target_status") or "").startswith("identified_")
        and _as_float(row.get("target_planned_qty"), 0.0) > 1e-12
    ]
    if not candidates:
        raise ValueError("No comparable positive op_93 smoke case exists")
    selected = min(
        candidates,
        key=lambda row: (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    smoke_point = args.operating_point_id or "op_93"
    if smoke and smoke_point != "op_93":
        raise ValueError("The V4 campaign smoke contract is fixed to op_93")
    manifest, points, lanes = prepare_manifest(
        ...
        # A smoke run uses the signed V4 bridge and completed 3-run design
        # registry.  Its execution_scope prevents reuse in campaign shards.
        smoke=False,
    ...
    smoke_lane_id = args.smoke_lane_id
    if smoke:
        smoke_registry = load_target_registry(
            output_dir=args.output_dir, manifest=manifest, lanes=lanes
        )
        smoke_seed, smoke_lane_id = select_smoke_case(
            smoke_registry,
            requested_seed=smoke_seed,
            requested_lane_id=smoke_lane_id,
        )
    result = run_shard(
        output_dir=args.output_dir,
        manifest=manifest,
        ...
        smoke_seed=smoke_seed if smoke else None,
        smoke_lane_id=smoke_lane_id if smoke else None,
