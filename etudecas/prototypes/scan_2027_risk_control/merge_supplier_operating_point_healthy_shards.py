#!/usr/bin/env python3
"""Merge current-engine op100 shards with strict evidence/provenance checks."""
...
DEFAULT_INPUTS = (
    ...
    / "supplier_operating_point_op100_current_full_20260904_v1",
    protocol.ARTIFACT_PARENT
    / "supplier_operating_point_op100_tail_20260904_v1",
)
DEFAULT_OUTPUT = (
    protocol.ARTIFACT_PARENT
...
def _evidence_key(payload: Mapping[str, Any]) -> tuple[str, str, str, int] | None:
    ...
    if point_id != "op_100" or seed != protocol.SCREENING_SEED:
        return None
    if not incident:
...
def merge(*, inputs: Sequence[Path], output: Path) -> None:
    ...
    for source in sources:
        detail_path = source / "op100_current_engine_audit.csv"
        for row in campaign._read_csv(detail_path):
            if (
                str(row["operating_point_id"]) != "op_100"
    ...
    # Rebuild the compact rows from their atomic evidence so interpretation-
    # only corrections also apply to cached cases produced by older runners.
    for key in list(merged):
        preferred_source = selected_source[key]
        incident_candidates = evidence_candidates.get(key) or []
        baseline_key = ("op_100", "__baseline__", "", key[3])
    ...
    expected_rows = 18 * len(campaign.MECHANISMS)
    if len(rows) != expected_rows:
        raise ValueError(f"Expected 36 op100 rows, found {len(rows)}")
    if len({row["chain_id"] for row in rows}) != 18:
        raise ValueError("Merged op100 screen does not contain 18 distinct lanes")
    if {row["incident_mechanism"] for row in rows} != set(campaign.MECHANISMS):
        raise ValueError("Merged op100 screen does not contain the same two causes")
    output.mkdir(parents=True, exist_ok=True)
    detail_output = output / "op100_current_engine_audit.csv"
    campaign._write_csv(detail_output, rows, campaign.DETAIL_FIELDS)
    summary_fields, summaries = campaign._summary_rows(rows)
    campaign._write_csv(
        output / "op100_current_engine_audit_summary.csv",
        summaries,
        summary_fields,
    )

    required_keys = set(merged) | {
        ("op_100", "__baseline__", "", protocol.SCREENING_SEED)
    }
    ...
    campaign._write_json(
        ...
        {
            "schema_version": "etudecas.supplier_healthy_shard_merge.v1",
            "status": "complete",
            "row_count": len(rows),
            "lane_count": 18,
            ...
            "inputs": [
                {
                    ...
                    "detail_csv_sha256": _sha256(
                        source / "op100_current_engine_audit.csv"
                    ),
                }
                for source in sources
            ],
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        dest="input_dirs",
        help="Repeat for each independent op100 shard.",
    )
