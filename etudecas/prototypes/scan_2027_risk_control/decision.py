...
def run_adaptive_controller(context: RunContext, config: Mapping[str, Any], actions: Sequence[Action], seed: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for day, input_row in frame.iterrows():
        one_step = no_uncertainty_scenario(1)
        state, metrics = simulate_step(state, current_action, float(input_row["demand"]), float(input_row["base_risk"]), one_step, 0, config)
        ...
        if day % review_period == 0:
            ...
            scenarios = sample_scenarios(int(config["controller_scenarios"]), horizon, config, seed + day)
...
def simulate_fixed_policy_scenarios(context: RunContext, config: Mapping[str, Any], actions: Sequence[Action], seed: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    scenarios = sample_scenarios(int(config["policy_comparison_scenarios"]), len(frame), config, seed + 9000)
    ...
    for action in actions:
        for scenario_id, scenario in enumerate(scenarios[: min(12, len(scenarios))]):
...
def regime_transition_matrix(trajectory: pd.DataFrame) -> pd.DataFrame:
    regimes = list(dict.fromkeys(trajectory["regime"].astype(str).tolist()))
